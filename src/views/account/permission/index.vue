      ...
      >
        ...
        <el-table-column label="Hành động" align="center">
          <template slot-scope="scope">
            ...
            <el-button type="danger" icon="el-icon-delete" circle @click="onDeletePermission(scope.row)" />
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    onDeletePermission(row) {
      this.$confirm(`Bạn có chắc chắn muốn xóa quyền ${row.descriptions} đã liên kết với các vai trò: ${row.roles.map(({ name }) => name).join(',')}`).then(_ => {
        if (isEmpty(row.roles)) {
          permissionResource.remove(row.id).then(res => {
            if (res.error_code === 0) {
              this.onGetList()
          ...
          this.$message.error('Vui lòng xóa hết liên kết đến vai trò liên quan')
        }
      })
    },
    onGetList() {
      this.loadingTable = true
      permissionResource.list(this.listBody, this.listQuery).then(res => {
