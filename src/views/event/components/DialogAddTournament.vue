  ...
  <el-dialog :title="dialog.titleName" :visible.sync="showDialog" width="50%" :before-close="handleClose">
    <el-form ref="ruleForm" v-loading="dialogLoading" :model="objectData" class="demo-ruleForm">
      <el-row>
        ...
        <el-col :sm="24" :md="24">
          <el-form-item label="Tên Tournament :" required>
            <el-form-item prop="name">
              <el-input
                v-model="formRequest.name"
                prop="name"
                placeholder="Tên Tournament"
...
export default {
  ...
  data() {
    return {
      ...
      facilityModel: {},
      file: null,
      dialog: { titleName: 'Thêm mới Tournament', buttonName: 'Thêm mới' },
      organization: null
    }
  },
  computed: {
  ...
  watch: {
    async objectData(value) {
      ...
      if (!this.isAdd) {
        ...
        this.listImage = value.img_poster ? [{ url: value.img_poster }] : []
      } else {
        if (value) {
          this.formRequest = {
            ...this.formRequest,
    ...
    isAdd(value) {
      ...
      if (value) {
        this.dialog = Object.assign({}, { titleName: 'Thêm mới Tournament', buttonName: 'Thêm mới' })
      } else {
        this.dialog = Object.assign({}, { titleName: 'Cập nhật Tournament', buttonName: 'Cập nhật' })
      }
    }
  },
  ...
  methods: {
    ...
    removeFile(value) {
      delete this.formRequest.img_poster
      this.file = null
    },
    previewFile(value) {
      if (!this.isAdd) {
    ...
    async appendForm() {
      ...
      if (this.file != null && isEmpty(formData.img_poster)) {
        const fileUpload = await this.uploadFileToServe(this.file)
        if (fileUpload) { formData.img_poster = fileUpload.data.name }
      }
