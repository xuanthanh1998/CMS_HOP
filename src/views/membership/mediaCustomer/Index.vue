  ...
  <el-row style="margin:20px;">
    <el-row class="list-account__header">
      <el-col :sm="24" style="display:flex;">
        <h1 style="margin: 0; width: 100%; padding: 10px; text-align: center;">Quản lý truyền thông khách hàng</h1>
      </el-col>
    </el-row>
    ...
    <el-row style="margin-top: 40px;">
      ...
      >
        ...
        >
          <template slot-scope="{row}">
            {{ row.golfer.id_display ? `vID${row.golfer.id_display}` : '' }}
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="{row}">
            <el-link :href=" row.link ? row.link : '' " target="_blank" type="success" class="text-wrap">
              {{ row.link ? row.link : '' }}
            </el-link>
          </template>
        </el-table-column>
...
import DialogAddMediaCustomer from '@/views/membership/mediaCustomer/components/DialogAddMediaCustomer.vue'
...
export default {
  ...
  methods: {
    ...
    onGetList() {
      ...
      listGolferResources.listMediaCustomer(this.query).then(response => {
